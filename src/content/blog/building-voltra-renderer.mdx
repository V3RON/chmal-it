...
import DispatcherSwap from '../../components/building-voltra-renderer/DispatcherSwap';
...
To accomplish this, Voltra relies on a custom renderer that transforms React Native JSX into JSON at runtime, without relying on build-time magic. The goal was simple: allow developers to modify the layout of their Live Activities and Widgets dynamically throughout the app's lifetime.

Initially, the Voltra renderer relied on `react-reconciler`, and for a while, it worked perfectly. However, when I began working on a server-side API, I hit a wall. I realized I needed to drop the standard `react-reconciler` in favor of a custom solution. 

Here is exactly why that happened, and how I built the replacement.
